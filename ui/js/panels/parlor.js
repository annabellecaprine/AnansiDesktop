...
CRITICAL: Respond ONLY with valid JSON:
{
  ...
  // ============================================
  function createActorFromParlor(name, personality, gender, extraNotes) {
      ...
      actions: [
        ...
        },
        {
          ...
          onclick: (modal) => {
            ...
            const companionName = modal.querySelector('#companion-selector-container')?.querySelector('div[style*="font-weight: 600"]')?.textContent?.replace('👥 Companion: ', '') || null;
            ...
            createActorFromParlor(name, personality, gender, 'Generated by The Spider\'s Parlor');

            // Create companion if present
            if (companionName && companionPersonality) {
              createActorFromParlor(companionName, companionPersonality, 'any', `Companion of ${name}`);
            }

            A.State.notify();
            if (A.UI.Toast) A.UI.Toast.show(`${companionName ? 'Actors' : 'Actor'} "${name}"${companionName ? ` and "${companionName}"` : ''} added!`, 'success');
      ...
      actions: [
        ...
        },
        {
          ...
          onclick: (modal) => {
            ...
            // Create an actor for each character
            ...
            for (let i = 0; i < charNames.length; i++) {
              ...
              const personality = charPersonalities[i]?.value.trim() || '';
              if (name) {
                createActorFromParlor(name, personality, i === 0 ? gender : 'any', 'Generated by The Spider\'s Parlor (Ensemble)');
                createdCount++;
              }
            }

            A.State.notify();
            if (A.UI.Toast) A.UI.Toast.show(`${createdCount} actors added to the web!`, 'success');
            A.UI.switchPanel('actors');
